...
# =============================================================================
    ...
    def _init_graph(self):
        ...
        self.cnn_label = tf.placeholder(tf.float32, [None,self.num_of_seq], name="cnnlabel")
# =============================================================================
...
# =============================================================================       
        ...
        '''h_trans is the input of lstm(another py project),we will print it to txt when we load the best model for testing
        '''
        self.h_trans =  tf.transpose(self.h_fc2,[1,0,2])
        shape =self. h_trans.get_shape().as_list()
        dim = np.prod(shape[1:]) 
        
        h_fc_flat= tf.reshape(self.h_trans, [-1,dim])
    ...
    def test(self, enl,enr,anl,anr,gnl,gnr,lnl,lnr,onl,onr,y_train,ohtr,tr_len,cnn_label):
        ...
        for start,end in zip(range(0,self.sjnum,self.batch_size),range(self.batch_size,self.sjnum+1,self.batch_size)):
            ...
            output=self.sess.run(self.h_trans, feed_dict={self.emgl:a,
                                                     self.emgr:a1,
                                                     self.accl:b,
                                                     self.accr:b1,
                                                     self.gyrl:c,
            ...
            for j in range(self.batch_size):
                for k in range(400):
                    for m in range(self.wordnum):
                        f.write(str(output[j,k,m])+' ')
                    f.write('\n')
                f.write('\n')
        ...
        e1=[]
        h=[]
        for j in range(self.sjnum+1-self.batch_size,self.sjnum+1):
            a.append(enl[j])
            a1.append(enr[j])
            b.append(anl[j])
            b1.append(anr[j])
        ...
        output=self.sess.run(self.h_trans, feed_dict={self.emgl:a,
                                                 ...
                                                 self.orir:e1,
                                                 self.cnn_label:h})    
        for j in range(self.batch_size+end-self.sjnum-1,self.batch_size):
            for k in range(400):
                for m in range(self.wordnum):
                    f.write(str(output[j,k,m])+' ')
                f.write('\n')
...
def test():
    ...
    model.test(data[0],data[2],data[4],data[6],data[8],data[10],data[12],data[14],data[16],data[18],data[20],data[22],data[23],cnn_label)
