...
# =============================================================================
        with tf.variable_scope("encoder"):
            self.s_proj_W = tf.get_variable("s_proj_W", shape=[28, self.num_of_hidden],
                                            initializer=initializer)
...
# =============================================================================        
        ...
        plogits = []
        pprobs  = []
        prob = tf.Variable(tf.constant(1/self.wordnum,shape=[self.batch_size,self.wordnum]))
        s = self.decoder.zero_state(self.batch_size, tf.float32)
        for t in range(self.word_em):
            if t > 0: tf.get_variable_scope().reuse_variables()
            x = prob
        ...
        with tf.variable_scope("1mmm",reuse=tf.AUTO_REUSE):
            ...
            self.preloss = tf.reduce_sum(self.lossa) + self.ploss
    ...
    def train(self, enl,enr,anl,anr,gnl,gnr,lnl,lnr,onl,onr,y_train,ohtr,tr_len):
        ...
        for i in range(1000):
            ...
            output=self.sess.run([self.maxa], feed_dict={self.emgl:a,
                                                 self.emgr:a1,
                                                 self.accl:b,
                                                 self.accr:b1,
                                                 self.gyrl:c,
            ...
            mineloss=0
            for j in range(self.batch_size):
                    temp=self.lcs(f[j],output[0][j],g[j],self.word_em)
                    totacc=totacc+temp
                    mineloss=mineloss+pow((3-temp),2)
            mineloss=10*mineloss/self.batch_size
            print('trainloss',mineloss)
            ...
            for start,end in zip(range(0,self.sjnum,self.batch_size),range(self.batch_size,self.sjnum+1,self.batch_size)):
                ...
                for j in range(start,end):
                    ...
                    f1.append(ohtr[j])
                    g.append(tr_len[j])
                maxa,rloss=self.sess.run([self.pmaxa,self.preloss], feed_dict={self.emgl:a,
                                                         self.emgr:a1,
                                                         self.accl:b,
                                                         self.accr:b1,
                                                         self.gyrl:c,
                ...
                for j in range(self.batch_size):
                    totacc=totacc+self.lcs(f[j],maxa[j],g[j],self.word_em)
                print('loss',rloss)
