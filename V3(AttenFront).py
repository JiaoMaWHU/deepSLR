...
class SLR:
    ...
    # def cond(self, W_a, W_x, W_t, multiCH, p12, i):
    #     print(i)
    #     return i <  tf.constant(400)
    #
    # def body(self, W_a, W_x, W_t, multiCH, p12, i):
    #     multiCH = tf.nn.softmax(
    #         tf.multiply(tf.nn.tanh(tf.multiply(W_a, multiCH) + tf.multiply(W_x, p12[i])), W_t))
    #     self.p1_t.append(tf.multiply(p12[i], multiCH))
    #     return W_a, W_x, W_t, multiCH, p12, i+1
    ...
    def _init_graph(self):
        ...
        # Attention Part
        W_a = self.weight_variable([100, 8], name="W_a")
        W_x = self.weight_variable([100, 8], name="W_x")
        W_t = self.weight_variable([100, 8], name="W_t")
        multiCH = tf.zeros([100, 8])
        p1_t = []
        for i in range(402):
            x = p12[i]
            multiCH = tf.nn.softmax(
        ...
        emgll = tf.reshape(tf.transpose(tf.stack(p1_t), [1, 0, 2]), [100, 402, 8, 1])

        W_conv1 = self.weight_variable([2, 2, 1, 3], name="W_conv1")
        b_conv1 = self.bias_variable([3])
        h_conv1 = tf.nn.relu(
            tf.nn.conv2d(emgll, W_conv1, strides=[1, 1, 1, 1], padding='VALID') + b_conv1)
        ...
        # Attention Part
        W_a1 = self.weight_variable([100, 8], name="W_a1")
        W_x1 = self.weight_variable([100, 8], name="W_x1")
        W_t1 = self.weight_variable([100, 8], name="W_t1")
        multiCH = tf.zeros([100, 8])
        p1_t = []
        for i in range(402):
            x = p12[i]
            multiCH = tf.nn.softmax(
                tf.multiply(tf.nn.tanh(tf.multiply(W_a1, multiCH) + tf.multiply(W_x1, x)), W_t1))
            p1_t.append(tf.multiply(x, multiCH))
        emgrl = tf.reshape(tf.transpose(tf.stack(p1_t), [1, 0, 2]), [100, 402, 8, 1])
        W_conv2 = self.weight_variable([2, 2, 1, 3], name="W_conv2")
        b_conv2 = self.bias_variable([3])
        h_conv2 = tf.nn.relu(
            tf.nn.conv2d(emgrl, W_conv2, strides=[1, 1, 1, 1], padding='VALID') + b_conv2)
...
def make_save_file(args):
    pretrain_path = './Model/%s_v3' % (args.dataset)
...
def make_log_file(args):
    pretrain_path = './Model/%s_v3.log' % (args.dataset)
    if not os.path.exists(pretrain_path):
