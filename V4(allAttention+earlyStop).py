...
def parse_args():
    ...
    parser.add_argument('--optimizer', nargs='?', default='Adagrad',
                        help='Specify an optimizer type (Adagrad, Adam, Ftrl, Momentum, RMSProp, SGD)')
...
class SLR:
    ...
    def set_front_attention(self, p12, dim, _range, _name):
        # Attention Part
        W_a = self.weight_variable([100, dim], name=(_name+'_W_a'))
        W_x = self.weight_variable([100, dim], name=(_name+'_W_x'))
        W_t = self.weight_variable([100, dim], name=(_name+'_W_t'))
        multiCH = tf.zeros([100, dim])
        p1_t = []
        ...
        return tf.reshape(tf.transpose(tf.stack(p1_t), [1, 0, 2]), [100, _range, dim, 1])
    ...
    def _init_graph(self):
        ...
        # =============================================================================
        shape = self.emgl.get_shape().as_list()
        dim = np.prod(shape[2:])
        p12 = tf.transpose(tf.reshape(self.emgl, [-1, 402, dim]), [1, 0, 2])  # ->
        _emgl = self.set_front_attention(p12, 8, 402, "emgl")
        W_conv1 = self.weight_variable([2, 2, 1, 3], name="W_conv1")
        b_conv1 = self.bias_variable([3])
        h_conv1 = tf.nn.relu(
            tf.nn.conv2d(_emgl, W_conv1, strides=[1, 1, 1, 1], padding='VALID') + b_conv1)
        p1 = tf.nn.max_pool(h_conv1, [1, 2, 2, 1], [1, 1, 1, 1], padding='VALID')
        # =============================================================================
        # Conv - MaxPooling - Emg right - 402*8=>400*6*3
        # =============================================================================
        shape = self.emgr.get_shape().as_list()
        dim = np.prod(shape[2:])
        p12 = tf.transpose(tf.reshape(self.emgr, [-1, 402, dim]), [1, 0, 2])  # ->
        # Attention Part
        _emgr = self.set_front_attention(p12, 8, 402, "emgr")
        W_conv2 = self.weight_variable([2, 2, 1, 3], name="W_conv2")
        b_conv2 = self.bias_variable([3])
        h_conv2 = tf.nn.relu(
            tf.nn.conv2d(_emgr, W_conv2, strides=[1, 1, 1, 1], padding='VALID') + b_conv2)
        p2 = tf.nn.max_pool(h_conv2, [1, 2, 2, 1], [1, 1, 1, 1], padding='VALID')
        # =============================================================================
        # Conv - MaxPooling - Acc left -  402*3=>400*1*1
        # =============================================================================
        shape = self.accl.get_shape().as_list()
        dim = np.prod(shape[2:])
        p12 = tf.transpose(tf.reshape(self.accl, [-1, 402, dim]), [1, 0, 2])  # ->
        _accl = self.set_front_attention(p12, 3, 402, 'accl')
        W_conv3 = self.weight_variable([2, 2, 1, 3], name="W_conv3")
        b_conv3 = self.bias_variable([3])
        h_conv3 = tf.nn.relu(
            tf.nn.conv2d(_accl, W_conv3, strides=[1, 1, 1, 1], padding='VALID') + b_conv3)
        p3 = tf.nn.max_pool(h_conv3, [1, 2, 2, 1], [1, 1, 1, 1], padding='VALID')
        # =============================================================================
        # Conv - MaxPooling - Acc right -  402*3=>400*1*1
        # =============================================================================
        shape = self.accr.get_shape().as_list()
        dim = np.prod(shape[2:])
        p12 = tf.transpose(tf.reshape(self.accr, [-1, 402, dim]), [1, 0, 2])  # ->
        _accr = self.set_front_attention(p12, 3, 402, 'accr')
        W_conv4 = self.weight_variable([2, 2, 1, 3], name="W_conv4")
        b_conv4 = self.bias_variable([3])
        h_conv4 = tf.nn.relu(
            tf.nn.conv2d(_accr, W_conv4, strides=[1, 1, 1, 1], padding='VALID') + b_conv4)
        p4 = tf.nn.max_pool(h_conv4, [1, 2, 2, 1], [1, 1, 1, 1], padding='VALID')

        # =============================================================================
        # Conv - MaxPooling - Gyr left -  402*3=>400*1*1
        # =============================================================================
        shape = self.gyrl.get_shape().as_list()
        dim = np.prod(shape[2:])
        p12 = tf.transpose(tf.reshape(self.gyrl, [-1, 402, dim]), [1, 0, 2])  # ->
        _gyrl = self.set_front_attention(p12, 3, 402,'gyrl')
        W_conv5 = self.weight_variable([2, 2, 1, 3], name="W_conv5")
        b_conv5 = self.bias_variable([3])
        h_conv5 = tf.nn.relu(
            tf.nn.conv2d(_gyrl, W_conv5, strides=[1, 1, 1, 1], padding='VALID') + b_conv5)
        p5 = tf.nn.max_pool(h_conv5, [1, 2, 2, 1], [1, 1, 1, 1], padding='VALID')

        # =============================================================================
        # Conv - MaxPooling - Gyr right -  402*3=>400*1*1
        # =============================================================================
        shape = self.gyrr.get_shape().as_list()
        dim = np.prod(shape[2:])
        p12 = tf.transpose(tf.reshape(self.gyrr, [-1, 402, dim]), [1, 0, 2])  # ->
        _gyrr = self.set_front_attention(p12, 3, 402, 'gyrr')
        W_conv6 = self.weight_variable([2, 2, 1, 3], name="W_conv6")
        b_conv6 = self.bias_variable([3])
        h_conv6 = tf.nn.relu(
            tf.nn.conv2d(_gyrr, W_conv6, strides=[1, 1, 1, 1], padding='VALID') + b_conv6)
        p6 = tf.nn.max_pool(h_conv6, [1, 2, 2, 1], [1, 1, 1, 1], padding='VALID')

        # =============================================================================
        # Conv - Ol left -  400*3=>400*1*1
        # =============================================================================
        shape = self.oll.get_shape().as_list()
        dim = np.prod(shape[2:])
        p12 = tf.transpose(tf.reshape(self.oll, [-1, 400, dim]), [1, 0, 2])  # ->
        _oll = self.set_front_attention(p12, 3, 400, 'oll')
        W_conv7 = self.weight_variable([1, 3, 1, 1], name="W_conv7")
        b_conv7 = self.bias_variable([1])
        h_conv7 = tf.nn.relu(
            tf.nn.conv2d(_oll, W_conv7, strides=[1, 1, 1, 1], padding='VALID') + b_conv7)

        # =============================================================================
        # Conv - Ol right -  400*3=>400*1*1
        # =============================================================================
        shape = self.olr.get_shape().as_list()
        dim = np.prod(shape[2:])
        p12 = tf.transpose(tf.reshape(self.olr, [-1, 400, dim]), [1, 0, 2])  # ->
        _olr = self.set_front_attention(p12, 3, 400, 'olr')
        W_conv8 = self.weight_variable([1, 3, 1, 1], name="W_conv8")
        b_conv8 = self.bias_variable([1])
        h_conv8 = tf.nn.relu(
            tf.nn.conv2d(_olr, W_conv8, strides=[1, 1, 1, 1], padding='VALID') + b_conv8)

        # =============================================================================
        # Conv - Ori left -  400*4=>400*1*1
        # =============================================================================
        shape = self.oril.get_shape().as_list()
        dim = np.prod(shape[2:])
        p12 = tf.transpose(tf.reshape(self.oril, [-1, 400, dim]), [1, 0, 2])  # ->
        _oril = self.set_front_attention(p12, 4, 400, 'oril')
        W_conv9 = self.weight_variable([1, 4, 1, 1], name="W_conv9")
        b_conv9 = self.bias_variable([1])
        h_conv9 = tf.nn.relu(
            tf.nn.conv2d(_oril, W_conv9, strides=[1, 1, 1, 1], padding='VALID') + b_conv9)

        # =============================================================================
        # Conv - Ori right -  400*4=>400*1*1
        # =============================================================================
        shape = self.orir.get_shape().as_list()
        dim = np.prod(shape[2:])
        p12 = tf.transpose(tf.reshape(self.orir, [-1, 400, dim]), [1, 0, 2])  # ->
        _orir = self.set_front_attention(p12, 4, 400, 'orir')
        W_conv10 = self.weight_variable([1, 4, 1, 1], name="W_conv10")
        b_conv10 = self.bias_variable([1])
        h_conv10 = tf.nn.relu(
            tf.nn.conv2d(_orir, W_conv10, strides=[1, 1, 1, 1], padding='VALID') + b_conv10)
        ...
        with tf.variable_scope("Encoder"):
            ...
            x = tf.split(x, 400)
            # lstm_cell_fw = cudnn_rnn.CudnnLSTM(num_layers=1, num_units=128, dropout=1 - self.dropout)
            # lstm_cell_bw = cudnn_rnn.CudnnLSTM(num_layers=1, num_units=128, dropout=1 - self.dropout)
        ...
        with tf.variable_scope("Decoder"):
            ...
            # self.decoder = cudnn_rnn.CudnnLSTM(num_layers=2, num_units=256, dropout=1 - self.dropout)
        ...
        with tf.variable_scope("Run_decoder", reuse=tf.AUTO_REUSE):
            ...
            self.optim = tf.contrib.layers.optimize_loss(self.loss, None,
                                                         self.lr_init, self.optimizer_type, clip_gradients=5.,
        ...
        for i in range(self.epoch):
            ...
            while num < self.batch_size:
                if data[arr[j]] in src:
                    emg_l.append(enl[arr[j]])
                    emg_r.append(enr[arr[j]])
                    acc_l.append(anl[arr[j]])
                    acc_r.append(anr[arr[j]])
                    gyr_l.append(gnl[arr[j]])
                    gyr_r.append(gnr[arr[j]])
                    ol_l.append(oll[arr[j]])
                    ol_r.append(olr[arr[j]])
                    ori_l.append(orl[arr[j]])
                    ori_r.append(orr[arr[j]])
                    target_data.append(y_train[arr[j]])
                    label_data.append(ohtr[arr[j]])
            ...
            for start, end in zip(range(0, self.sdnum, self.batch_size),
                ...
                for j in range(start, end):
                    emg_l.append(cdata[0][j])
                    emg_r.append(cdata[1][j])
                    acc_l.append(cdata[2][j])
                    acc_r.append(cdata[3][j])
                    gyr_l.append(cdata[4][j])
                    gyr_r.append(cdata[5][j])
                    ol_l.append(cdata[6][j])
                    ol_r.append(cdata[7][j])
                    ori_l.append(cdata[8][j])
                    ori_r.append(cdata[9][j])
                    target_data.append(y_test[j])
                    label_data.append(ohte[j])
                    g.append(te_len[j])
                prob, rloss = self.sess.run([self.pprobs, self.ploss], feed_dict={self.emgl: emg_l,
                                                                                  self.emgr: emg_r,
                                                                                  self.accl: acc_l,
                                                                                  self.accr: acc_r,
                                                                                  self.gyrl: gyr_l,
                ...
                for k in range(self.batch_size):
                    ...
                    aq, isr, idr = self.lcs(target_data[k][1:], hb_maxa, g[k] - 1, len(hb_maxa))
                    totacc = totacc + aq
                    totir = totir + isr
                    totdr = totdr + idr
            totacc = totacc / end
            ...
            if totacc < self.totacc_last:
                self.early_stop_number = self.early_stop_number + 1
            elif totacc >= self.totacc_last:
                self.early_stop_number = 0

            self.totacc_last = totacc
            if self.early_stop_number > 4:
            ...
            # with open('./Timeline/timeline_%d.json'%(i), 'w') as f:
...
def make_save_file(args):
    pretrain_path = './Model/%s_v4' % (args.dataset)
...
def make_log_file(args):
    pretrain_path = './Model/%s_v4.log' % (args.dataset)
    if not os.path.exists(pretrain_path):
